            ...
            <tbody class="divide-y divide-gray-200">
                @foreach ($directores as $director)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">{{ $director->nombre }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">{{ $director->email }}</td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap flex gap-2">
                            <button wire:click="edit({{ $director->id }})" class="text-indigo-600 hover:text-indigo-900 text-sm">Editar</button>
                            @if($director->activo)
                                <button wire:click="deactivate({{ $director->id }})" class="text-red-600 hover:text-red-900 text-sm">Desactivar</button>
                            @else
                                <button wire:click="activate({{ $director->id }})" class="text-green-600 hover:text-green-900 text-sm">Reactivar</button>
                            @endif
                        </td>
                    </tr>
                @endforeach
